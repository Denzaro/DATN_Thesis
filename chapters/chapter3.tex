...
\begin{itemize}
    ...
    \item Cho phép so sánh hiệu quả giữa chiến lược điều khiển thích ứng và chiến lược cố định.
\end{itemize}
...
\hspace*{1cm} Quá trình thu thập dữ liệu bắt đầu bằng việc khởi tạo Google Drive API để kết nối và thao tác với hệ thống lữu trữ đám mây. Tiếp theo, hệ thống sẽ khởi tạo một hàng đợi dùng để tải ảnh lên Drive (drive_queue) và một bộ nhớ đệm để lưu trữ mã SHA của ảnh được lưu gần nhất của mỗi camera. Sau đó, hệ thống sẽ tiến hành chạy đa luồng để thực hiện các tác vụ sau:
\begin{itemize}
    \item \textbf{Nhóm luồng giám sát camera: } Ở nhóm luồng này tùy theo số lượng camera cần giám sát là 13 camera ứng với 13 địa điểm mà hệ thống sẽ khởi tạo tương ứng số luồng. Mỗi luồng sẽ chịu trách nhiệm truy suất ảnh từ một camera cụ thể. Sau đó, cứ mỗi khoảng thời gian nhất định là khoảng 5 phút thì ảnh sẽ được gom lại và gửi vào hàng đợi drive\_queue để chờ được tải lên Google Drive.
    \item \textbf{Luồng chính: } Luồng này có đóng vai trò như một bộ điều phối, chịu trách nhiệm phân loại và giao task cho nhóm luồng upload ảnh lên Drive.
...
\subsection{Thiết kế và thực hiện các agent điều khiển giao thông}
...
\subsubsection{DurationAgent: Dự báo Thời Lượng Xanh}
...
Tại mỗi time step, input $x_t^{\text{norm}}$ cùng với hidden state trước đó $h_{t-1}$ và cell state $c_{t-1}$ được đưa vào khối LSTM. Theo lý thuyết trong Chương 2, Mục 2.1.1, khối LSTM thực hiện quá trình với ba cổng điều khiển:
...
\subsubsection{LaneRatioAgent: Phân Bổ Tỷ Lệ Làn Đường}
...
\subsection{Tích hợp với SUMO qua TraCI}

\subsubsection{Trích xuất State từ Sensors}
...
\subsubsection{Áp dụng Action vào Tín Hiệu Đèn Giao Thông}
...
\subsubsection{Huấn luyện Parallel với 16 Workers}
...
Quy trình phát hiện tắc nghẽn giao thông gồm 5 bước chính:
...
\textbf{Tại Sao Chọn Bộ Tối Ưu Adam?}
\begin{itemize}
