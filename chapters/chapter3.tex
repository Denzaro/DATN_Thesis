...
\begin{itemize}
    ...
    \item Lưu trữ kết quả đếm phương tiện với timestamp và vị trí tương ứng.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Khối điều chỉnh tín hiệu đèn giao thông: } Đầu tiên khối này dự báo lưu lượng giao thông trong tương lai dựa trên dữ liệu lịch sử từ khối nhận diện. Dữ liệu đếm phương tiện được chuyển đổi thành chuỗi thời gian và đưa vào mô hình LSTM (Long Short-Term Memory) để học các quy luật biến thiên của luồng giao thông. Trên cơ sở kết quả được đưa ra, khối này tính toán các thông số điều khiển tín hiệu tối ưu và gửi lệnh thông qua giao diện TraCI (Traffic Control Interface) để điều chỉnh chương trình đèn giao thông trong mô phỏng SUMO, nhằm tối ưu hóa khả năng thông hành của mạng lưới giao thông.
...
Kế đến, hệ thống sẽ tổng hợp các khung thời điểm về mỗi năm phút để tạo thành chuỗi thời gian lưu lượng giao thông đồng nhất. Quá trình này bao gồm việc nhóm các bản ghi theo khoảng thời gian năm phút, sau đó tính lượng trung bình phương tiện có trong khoảng thời gian đó. 

Cuối cùng, hệ thống sẽ thực hiện quá trình tinh chỉnh dữ liệu trước khi đưa vào hai khối điều khiển tín hiệu đèn giao thông và mô phỏng. Việc ùn tắc tại nút giao hoặc đường kính nút giao lớn làm một số phương tiện có thể xuất hiện trên 1 hoặc nhiều snapshot liên tiếp. Do đó, để tránh việc đếm số lượng phương tiện bị trùng lặp, hệ thống sẽ áp dụng thuật toán lọc trung bình như sau:
\begin{equation}
q_i(t) =
\left(
\frac{\bar{N}(t)\cdot 3600}{L_{\text{tot}}}
\cdot
\frac{L_i}{T_i}
\right)
\cdot
...
\text{Trong đó:}
\begin{itemize}
    \item $q_i(t)$ là lưu lượng phương tiện của nhánh $i$ tại thời điểm $t$ (xe/giờ),
    \item $\bar{N}(t)$ là số lượng phương tiện trung bình quan sát được tại nút giao trong khoảng thời gian $\Delta t$,
    \item $L_{\text{tot}}$ là tổng số làn (đã quy đổi) kết nối với nút giao,
    \item $L_i$ là số làn của nhánh $i$,
    \item $T_i$ là thời gian trung bình để phương tiện vượt qua nút giao từ nhánh $i$,
    \item $\gamma_i$ là hệ số tắc nghẽn, biểu diễn tỷ lệ phương tiện không tham gia dòng chảy.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Lựa chọn vùng (Location Selection):} Người dùng xác định khu vực nghiên cứu thông qua việc vẽ đa giác (Polygon) trên bản đồ tương tác (như hình \ref{fig:map_step1}). Tọa độ của đa giác này là dữ liệu đầu vào để truy xuất hạ tầng từ OpenStreetMap.
    \item \textbf{Xử lý dữ liệu OSM (OSM Processing):} Hệ thống sử dụng thư viện OSMnx để tải dữ liệu mạng lưới đường bộ. Tại giai đoạn này, một bước quan trọng là làm sạch dữ liệu (Network Cleanup) bằng cách loại bỏ các đoạn đường không phục vụ mô phỏng chính như đường hẻm nhỏ, đường cụt hoặc lối đi bộ để giảm tải tính toán (như hình \ref{fig:map_step2}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{fig/map_creation_step1_polygon.png}
    \caption{Giao diện lựa chọn vùng nghiên cứu và xác định đa giác giới hạn}
    \label{fig:map_step1}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{fig/map_creation_step2_cleanup.png}
        \caption{Bước làm sạch mạng lưới (xóa đường thừa)}
        \label{fig:map_step2}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{fig/map_creation_step3_export.png}
        \caption{Bước xuất dữ liệu và biên dịch sang SUMO}
        \label{fig:map_step3}
    \end{minipage}
\end{figure}

Hình \ref{fig:map_step1}, \ref{fig:map_step2} và \ref{fig:map_step3} minh họa giao diện của công cụ "SUMO Map Editor" được phát triển trong đề tài. Công cụ này cho phép người dùng tùy chỉnh bản đồ trước khi xuất sang định dạng SUMO thông qua các chức năng:
\begin{itemize}
...
\subsubsection{Hiệu chỉnh lưu lượng giao thông từ dữ liệu thực tế}

Một trong những thách thức lớn nhất trong mô phỏng giao thông là đảm bảo tính chân thực của dữ liệu đầu vào. Thay vì sử dụng các giá trị lưu lượng ngẫu nhiên hoặc giả định, đề tài xây dựng quy trình hiệu chỉnh (calibration) lưu lượng giao thông dựa trên dữ liệu thu thập thực tế từ khối nhận diện phương tiện.

\paragraph{Liên kết giữa khối thu thập dữ liệu và khối mô phỏng} \mbox{}\\

Dữ liệu số lượng phương tiện từ khối nhận diện (Chương 4, Mục 4.2) được sử dụng làm đầu vào để ước lượng lưu lượng giao thông tại các nút giao trong SUMO. Quy trình chuyển đổi bao gồm các bước sau:

\begin{enumerate}
    \item \textbf{Tổng hợp dữ liệu snapshot:} Từ các ảnh giao thông được xử lý (bao gồm cả phân đoạn và tính phần trăm che phủ), hệ thống thu được số lượng xe hai bánh và xe bốn bánh trung bình mỗi 5 phút tại từng địa điểm.
    
    \item \textbf{Ước lượng số lượng phương tiện:} Dựa vào quy ước chuyển đổi phần trăm che phủ (như hình 4.3.12), hệ thống ước lượng tổng số xe. Ví dụ: với phần trăm che phủ 40.6\% và quy ước 1\%/xe, ta ước lượng được $\approx 41$ xe hai bánh trong vùng đó.
    
    \item \textbf{Chuyển đổi sang tốc độ dòng chảy:} Số lượng xe trung bình $N(t)$ trong khoảng thời gian $\Delta t$ được chuyển đổi sang tốc độ dòng chảy (flow rate) theo công thức:
\end{enumerate}

\begin{equation}
q_i(t) = \left(\frac{N(t) \cdot 3600}{L_{\text{tot}}} \cdot \frac{L_i}{T_i}\right) \cdot (1 - \gamma_i)
\end{equation}

Trong đó:
\begin{itemize}
    \item $q_i(t)$ là lưu lượng phương tiện của nhánh $i$ tại thời điểm $t$ (xe/giờ),
    \item $N(t)$ là số lượng phương tiện trung bình quan sát được tại nút giao trong khoảng thời gian $\Delta t$,
    \item $L_{\text{tot}}$ là tổng số làn (đã quy đổi) kết nối với nút giao,
    \item $L_i$ là số làn của nhánh $i$,
    \item $T_i$ là thời gian trung bình để phương tiện vượt qua nút giao từ nhánh $i$,
    \item $\gamma_i$ là hệ số tắc nghẽn, biểu diễn tỷ lệ phương tiện không tham gia dòng chảy (ví dụ: $\gamma_i = 0.1$ nghĩa là 10\% phương tiện bị kẹt).
\end{itemize}

\paragraph{Ví dụ minh họa} \mbox{}\\

Giả sử tại nút giao Đinh Bộ Lĩnh - Bạch Đằng, dữ liệu snapshot cho thấy:
\begin{itemize}
    \item Tổng số xe hai bánh: 47 xe (từ phân đoạn + YOLOv11)
    \item Tổng số xe bốn bánh: 12 xe
    \item Tổng số làn: $L_{\text{tot}} = 16$ làn
    \item Nhánh hướng Bắc có $L_i = 4$ làn, $T_i = 15$ giây, $\gamma_i = 0.05$
\end{itemize}

Áp dụng công thức:
\begin{equation*}
q_{\text{Bắc}} = \left(\frac{59 \cdot 3600}{16} \cdot \frac{4}{15}\right) \cdot (1 - 0.05) \approx 3363 \text{ xe/giờ}
\end{equation*}

Giá trị này được sử dụng để cấu hình luồng phương tiện (flow) trong tệp \texttt{flows.add.xml} của SUMO, đảm bảo mô phỏng phản ánh đúng tình trạng giao thông thực tế.

\paragraph{Lợi ích của phương pháp hiệu chỉnh} \mbox{}\\

\begin{itemize}
    \item \textbf{Tính chân thực cao:} Lưu lượng mô phỏng dựa trên dữ liệu quan sát thực tế, không phải giả định.
    \item \textbf{Tự động hóa:} Quy trình chuyển đổi từ snapshot sang flow config được tự động hóa hoàn toàn.
    \item \textbf{Linh hoạt theo thời gian:} Có thể tạo các kịch bản mô phỏng khác nhau cho từng khung giờ (giờ cao điểm, giờ thấp điểm) dựa trên dữ liệu thu thập.
\end{itemize}


\subsection{Thiết kế khối điều chỉnh tín hiệu đèn giao thông}

\subsubsection{Giới thiệu về dự báo lưu lượng trong điều chỉnh tín hiệu đèn}

Dự báo lưu lượng giao thông chịu trách nhiệm dự báo mức độ tắc nghẽn trong tương lai dựa trên dữ liệu lịch sử từ khối nhận diện phương tiện. Kết quả dự báo được sử dụng để điều chỉnh tín hiệu đèn giao thông thông qua giao diện TraCI, nhằm tối ưu hóa khả năng thông hành của mạng lưới.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth, height=0.75\textheight, keepaspectratio]{fig/gridlock_detection.png}
...
2 & Dropout(0,2) & (batch, 10, 64) \\
3 & LSTM(32) & (batch, 32) \\
4 & Dropout(0,2) & (batch, 32) \\
5 & Dense(1) & (batch, 1) \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Dropout (0,2):} Ngẫu nhiên tắt 20\% neuron, ngăn chặn quá khớp với tập dữ liệu nhỏ
    \item \textbf{Tầng 3 (LSTM 32 neuron):} Nén thông tin thành biểu diễn 32 chiều
    \item \textbf{Dropout (0,2):} Ngẫu nhiên tắt 20\% neuron, ngăn chặn quá khớp với tập dữ liệu nhỏ
    \item \textbf{Dense (1 neuron):} Ánh xạ từ biểu diễn thành 1 đầu ra (dự báo tắc toàn diện)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Tỷ Lệ Dropout:} 0,2 (bỏ 20\% neuron) - tiêu chuẩn cho LSTM với tập dữ liệu nhỏ
...
Hệ thống điều khiển giao thông thông minh được xây dựng theo kiến trúc phân cấp với hai agent độc lập chuyên biệt, phối hợp hoạt động để tối ưu hóa cả về mặt thời gian (temporal) và không gian (spatial).

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{fig/hierarchical_architecture.png}
    \caption{Kiến trúc hệ thống đa agent phân cấp (Hierarchical Multi-Agent System)}
    \label{fig:hierarchical_architecture}
\end{figure}

Như minh họa trong Hình \ref{fig:hierarchical_architecture}, kiến trúc bao gồm:
\begin{itemize}
    \item \textbf{DurationAgent:} Chịu trách nhiệm quyết định thời gian đèn xanh cho mỗi pha tín hiệu. Agent này sử dụng mạng nơ-ron LSTM kết hợp cơ chế Attention để phân tích chuỗi thời gian của các đặc trưng giao thông, từ đó đưa ra quyết định tối ưu dựa trên xu hướng tắc nghẽn.
    \item \textbf{LaneRatioAgent:} Chịu trách nhiệm phân bổ tỷ lệ thời gian giữa các làn đường, giúp cân bằng lưu lượng và giảm thiểu hiện tượng nghẽn cục bộ. Agent này hoạt động dựa trên phân phối Beta (Beta Distribution) để đảm bảo tính ngẫu nhiên có kiểm soát trong quá trình khám phá không gian hành động.
\end{itemize}

\paragraph{DurationAgent: Dự báo Thời Lượng Xanh với Cơ Chế Attention}

DurationAgent là thành phần cốt lõi chịu trách nhiệm điều khiển thời gian (temporal control). Trong phiên bản cải tiến này, chúng tôi nâng cấp kiến trúc từ LSTM tiêu chuẩn sang mạng LSTM kết hợp cơ chế chú ý (Attention Mechanism) để tăng cường khả năng nhận diện các biến động giao thông quan trọng trong chuỗi ký ức.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{fig/duration_agent_flow.png}
    \caption{Lưu đồ xử lý và kiến trúc chi tiết của DurationAgent}
    \label{fig:duration_agent_flow}
\end{figure}

\textbf{1. Đặc trưng đầu vào (Input Features):}

Thay vì chỉ sử dụng thông tin cơ bản, hệ thống sử dụng một vector đặc trưng 6 chiều ($D=6$) để mô tả toàn diện trạng thái giao thông tại nút giao:

\begin{equation}
x_t = [\text{throughput\_rate}, \text{queue\_delta}, \text{log\_queue}, \text{traffic\_variance}, \text{utilization}, \text{norm\_duration}]
\end{equation}

Trong đó:
\begin{itemize}
    \item \textbf{Throughput Rate:} Tỷ lệ xe thông qua nút giao trên khả năng thông hành lý thuyết.
    \item \textbf{Queue Delta:} Sự thay đổi của chiều dài hàng đợi so với bước thời gian trước đó (đạo hàm bậc nhất của hàng đợi).
    \item \textbf{Log-scaled Queue:} Giá trị logarit của hàng đợi ($\log(1 + Q)$), giúp khuếch đại tín hiệu khi hàng đợi còn thấp, cho phép agent phản ứng sớm hơn.
    \item \textbf{Traffic Variance:} Phương sai của lưu lượng giao thông, đo lường mức độ bất ổn định của dòng xe.
    \item \textbf{Utilization:} Tỷ lệ phần trăm thời gian đèn xanh được sử dụng hiệu quả.
    \item \textbf{Normalized Duration:} Thời lượng pha hiện tại đã được chuẩn hóa.
\end{itemize}

\textbf{2. Kiến trúc mạng LSTM + Attention:}

Mô hình sử dụng mạng LSTM 2 lớp với 128 hidden units (tăng từ 64) để trích xuất đặc trưng chuỗi thời gian (sequence length = 5). Điểm đặc biệt là sự tích hợp của Cơ chế Attention (Attention Mechanism) với hai chế độ hoạt động:

\begin{itemize}
    \item \textbf{Balanced Mode (Chế độ cân bằng):} Khi giao thông ổn định, sự chú ý được phân bổ đều cho các bước thời gian trong quá khứ.
    \item \textbf{Recent-Focused Mode (Chế độ tập trung hiện tại):} Khi phát hiện sự gia tăng đột ngột của hàng đợi (\textit{queue delta} lớn), cơ chế Attention sẽ tự động kích hoạt trọng số suy giảm theo hàm mũ, tập trung mạnh vào các quan sát gần nhất để phản ứng kịp thời với tình huống tắc nghẽn mới phát sinh.
\end{itemize}

\textbf{3. Cơ chế An toàn và Ổn định (Safety & Stability Mechanisms):}

Để đảm bảo tín hiệu đèn giao thông hoạt động an toàn và tránh dao động cực đoan, chúng tôi tích hợp các cơ chế xử lý hậu học (post-processing):

\begin{itemize}
    \item \textbf{Emergency Override (Can thiệp khẩn cấp):} Nếu hàng đợi vượt quá 15 phương tiện, hệ thống tự động cộng thêm một lượng "boost" (+0.2 vào hệ số thời gian) bất kể đầu ra của mạng nơ-ron, đảm bảo giải phóng nhánh đang bị ùn tắc nghiêm trọng.
    \item \textbf{Dynamic Delta Constraint:} Giới hạn mức độ thay đổi của thời lượng đèn giữa hai chu kỳ liên tiếp. Ngưỡng thay đổi này không cố định mà biến thiên dựa trên sự khẩn cấp của giao thông, nằm trong khoảng [0.15, 0.30].
    \item \textbf{Adaptive Smoothing:} Sử dụng kỹ thuật làm mượt theo hàm mũ (Exponential Smoothing) với hệ số $\alpha$ thích nghi. Khi giao thông biến động mạnh (phương sai lớn), $\alpha$ tăng lên (0.8) để phản ứng nhanh. Khi giao thông ổn định, $\alpha$ giảm xuống (0.6) để giữ tín hiệu đèn mượt mà.
\end{itemize}

\textbf{4. Đầu ra:}

Đầu ra của mô hình là một hệ số (coefficient) trong khoảng [0, 1], được chuyển đổi thành thời gian thực tế:
\begin{equation}
\text{Duration} = T_{\min} + \text{coef} \times (T_{\max} - T_{\min})
\end{equation}
Với $T_{\min} = 15$s và $T_{\max} = 120$s, đảm bảo thời gian xanh luôn nằm trong ngưỡng an toàn hợp lý.

\paragraph{LaneRatioAgent: Phân Bổ Tỷ Lệ Làn Đường}

LaneRatioAgent giải quyết bài toán phân bổ không gian (spatial allocation) bằng cách điều chỉnh tỷ lệ ưu tiên giữa các làn đường.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{fig/lane_ratio_agent_flow.png}
    \caption{Sơ đồ luồng hoạt động của LaneRatioAgent sử dụng Beta Distribution}
    \label{fig:lane_ratio_agent_flow}
\end{figure}

\textbf{Kiến trúc mạng Policy (Actor):}

Khác với cách tiếp cận quyết định rời rạc thông thường, LaneRatioAgent sử dụng \textbf{Beta Distribution} để mô hình hóa hành động liên tục. Mạng Policy (MLP) nhận trạng thái giao thông $s_t$ và sinh ra hai tham số thống kê đặc trưng cho phân phối xác suất:

\begin{equation}
\alpha = \text{softplus}(W_\alpha \cdot h + b_\alpha) + 1.0
\end{equation}
\begin{equation}
\beta = \text{softplus}(W_\beta \cdot h + b_\beta) + 1.0
\end{equation}
...
\paragraph{Hàm Phần Thưởng (Reward Function)} \mbox{}\\

Hàm phần thưởng đóng vai trò quan trọng trong việc định hướng hành vi của agent. Hệ thống sử dụng cơ chế \textbf{Queue-Based Target Reward} (Phần thưởng mục tiêu dựa trên hàng đợi) thay vì các công thức tuyến tính truyền thống.

Mục tiêu của hàm này là tính toán một hệ số thời gian mục tiêu ($Coef_{target}$) dựa trên chiều dài hàng đợi hiện tại:

\begin{itemize}
    \item Nếu hàng đợi thấp ($<3$ xe): $Coef_{target} = 0.1$ (Thời gian ngắn để tiết kiệm chu kỳ).
    \item Nếu hàng đợi trung bình ($3-10$ xe): $Coef_{target}$ tăng tuyến tính từ 0.1 đến 0.4.
    \item Nếu hàng đợi cao ($>10$ xe): $Coef_{target}$ tăng mạnh từ 0.4 đến 1.0 để giải phóng kẹt xe.
\end{itemize}

Phần thưởng cuối cùng được tính dựa trên độ lệch (deviation) giữa hành động thực tế và mục tiêu lý thuyết:

\begin{equation}
R = -200 \times (Coef_{actual} - Coef_{target})^2 + \text{Bonus}
\end{equation}

Việc sử dụng hình phạt bậc hai (-200) thay vì bậc nhất giúp tạo ra tín hiệu huấn luyện mạnh mẽ hơn gấp nhiều lần, buộc agent phải hội tụ nhanh chóng về chính sách tối ưu. Ngoài ra, một khoản điểm thưởng (+50) được trao tặng nếu sai số nằm trong ngưỡng chấp nhận được (dưới 0.15), khuyến khích sự ổn định.
...
\begin{itemize}
    ...
    \item \textbf{Công cụ đánh giá (Real-time Evaluation):} Hệ thống tích hợp module đánh giá thời gian thực thông qua giao diện TraCI, thay vì dựa vào các tệp log tĩnh. Dữ liệu vận hành (chiều dài hàng đợi, lưu lượng thông qua, phần thưởng tích lũy) được thu thập tại từng bước mô phỏng và lưu trữ dưới dạng bản ghi chi tiết (JSONL). Cơ chế này cho phép phân tích sâu hành vi ra quyết định của Agent (hệ số thời gian, phân bổ làn) và truy vết nguyên nhân các sự cố tắc nghẽn.
\end{itemize}
