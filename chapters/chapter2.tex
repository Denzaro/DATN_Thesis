...
Input Gate chịu trách nhiệm quyết định thông tin mới nào sẽ được thêm vào Cell State.Có thể nhìn thấy theo hình \ref{fig:lstm_input_gate}, quá trình cập nhật thông tin mới bao gồm sự phối hợp giữa Input Gate và vector ứng viên mới. Cụ thể:
\begin{itemize}
...
Đầu ra của hai bước trên sẽ được sử dụng để cập nhật Cell State hiện tại ($c_t$) như sau:
\begin{equation}
...
Việc cập nhật dữ liệu cho Cell State đã hoàn tất, bước tiếp theo cũng là bước cuối cùng trong quá trình xử lý của LSTM là quyết định một lớp ẩn mới. Để làm điều này, LSTM sử dụng: Cell State mới, lớp ẩn trước đó và dữ liệu đầu vào hiện tại.
...
Học tăng cường (Reinforcement Learning - RL) là một lĩnh vực của học máy tập trung vào việc huấn luyện các tác nhân (agents) học cách đưa ra quyết định tối ưu thông qua tương tác với một môi trường. Khác với học có giám sát (supervised learning) nơi có sẵn nhãn đúng cho mỗi đầu vào, hay học không giám sát (unsupervised learning) nơi tìm kiếm các mẫu ẩn, học tăng cường dựa trên phản hồi của môi trường dưới dạng phần thưởng (reward) hoặc hình phạt. Ý tưởng cốt lõi là agent học cách tối đa hóa tổng phần thưởng nhận được trong quá trình tương tác với môi trường. \cite{SuttonRL2018}

Các thành phần chính của một bài toán học tăng cường bao gồm:
\begin{itemize}
    \item \textbf{Tác nhân (Agent)}: Thực thể đưa ra quyết định và thực hiện các hành động trong môi trường.
    \item \textbf{Môi trường (Environment)}: Hệ thống mà agent tương tác, cung cấp trạng thái (state) và phần thưởng (reward) phản hồi lại các hành động của agent.
    \item \textbf{Trạng thái (State)}: Biểu diễn hiện tại của môi trường, mô tả đầy đủ thông tin mà agent cần để đưa ra quyết định.
    \item \textbf{Hành động (Action)}: Các quyết định mà agent có thể thực hiện để tác động đến môi trường.
    \item \textbf{Phần thưởng (Reward)}: Tín hiệu scalar phản ánh mức độ tốt hoặc xấu của hành động agent vừa thực hiện.
    \item \textbf{Chính sách (Policy)}: Hàm ánh xạ từ trạng thái đến hành động, biểu diễn chiến lược của agent.
\end{itemize}

Trong nhiều bài toán thực tế, học tăng cường được sử dụng để giải quyết các vấn đề điều khiển, tối ưu hóa và quy hoạch. Ví dụ, trong lĩnh vực giao thông, RL có thể được áp dụng để huấn luyện các bộ điều khiển tín hiệu giao thông thích ứng, giúp tối ưu hóa thông lượng xe và giảm thời gian chờ. Với khả năng học trực tiếp từ tương tác với môi trường, RL không yêu cầu một mô hình toán học tường minh của môi trường, điều này rất hữu ích khi môi trường phức tạp hoặc khó lập mô hình.

\subsection{Policy Gradient và Actor-Critic Methods}

Policy Gradient là một lớp các thuật toán học tăng cường tối ưu hóa trực tiếp chính sách (policy) của agent. Thay vì ước lượng hàm giá trị (value function), Policy Gradient học một chính sách xác suất $\pi_\theta(a|s)$ tham số hóa bởi $\theta$, biểu diễn xác suất chọn hành động $a$ khi ở trong trạng thái $s$. Mục tiêu là tối đa hóa kỳ vọng phần thưởng tích lũy:
...
Trong đó:
\begin{itemize}
    \item $J(\theta)$: Hàm mục tiêu (expected return).
    \item $\gamma$: Hệ số giảm giá (discount factor), $0 \leq \gamma \leq 1$.
    \item $r_t$: Phần thưởng tại bước thời gian $t$.
    \item $T$: Chiều dài tối đa của quỹ đạo (episode).
\end{itemize}

Policy Gradient sử dụng định lý Policy Gradient, cho phép tính toán gradient của hàm mục tiêu:
...
Trong đó:
\begin{itemize}
    \item $\nabla_\theta \log \pi_\theta(a|s)$: Gradient của log-xác suất hành động.
    \item $Q^\pi(s, a)$: Hàm giá trị hành động (action-value function), biểu diễn kỳ vọng phần thưởng khi thực hiện hành động $a$ ở trạng thái $s$ và sau đó tuân theo chính sách $\pi$.
\end{itemize}

\textbf{Actor-Critic Methods} là một biến thể của Policy Gradient kết hợp hai mạng nơ-ron:

\begin{itemize}
    \item \textbf{Actor (Diễn viên)}: Một mạng nơ-ron chính sách $\pi_\theta(a|s)$ học cách chọn hành động tốt nhất.
    \item \textbf{Critic (Nhà phê bình)}: Một mạng nơ-ron giá trị $V_\phi(s)$ ước lượng giá trị của trạng thái, được sử dụng để giảm phương sai của ước lượng gradient.
\end{itemize}

Advantage function được tính như sau:
...
Trong đó:
\begin{itemize}
    \item $A(s, a)$: Advantage (lợi thế), đo mức độ tốt hơn của hành động $a$ so với hành động trung bình tại trạng thái $s$.
    \item $r$: Phần thưởng ngay tức khắc.
    \item $\gamma V(s')$: Giá trị ước lượng của trạng thái tiếp theo.
    \item $V(s)$: Giá trị ước lượng của trạng thái hiện tại.
\end{itemize}

Policy Loss (Actor Loss):
\begin{equation}
\mathcal{L}_{\text{policy}} = -\mathbb{E}[\log \pi_\theta(a|s) \cdot A(s, a)]
\end{equation}

Value Loss (Critic Loss):
\begin{equation}
\mathcal{L}_{\text{value}} = \mathbb{E}[(V_\phi(s) - G_t)^2]
\end{equation}

Trong đó $G_t$ là return thực tế từ thời điểm $t$ trở đi.

\textbf{Ưu điểm của Actor-Critic:}
\begin{itemize}
    \item Giảm phương sai so với Policy Gradient thuần túy nhờ sử dụng Critic.
    \item Hội tụ nhanh hơn trong nhiều bài toán.
    \item Linh hoạt cho cả hành động liên tục (continuous actions) và rời rạc (discrete actions).
\end{itemize}

\subsection{Beta Distribution cho Hành Động Liên Tục}

Trong các bài toán mà agent cần chọn hành động liên tục (continuous actions) thay vì hành động rời rạc (discrete actions), cần một cách để mô hình hóa phân bố xác suất trên một không gian liên tục. Beta Distribution là một lựa chọn phổ biến khi hành động nằm trong khoảng $[0, 1]$ hoặc có thể được chuẩn hóa vào khoảng đó.

Beta Distribution được tham số hóa bởi hai tham số hình dạng $\alpha$ và $\beta$:

\begin{equation}
\text{Beta}(\alpha, \beta) = \frac{\Gamma(\alpha + \beta)}{\Gamma(\alpha)\Gamma(\beta)} x^{\alpha - 1}(1 - x)^{\beta - 1}
\end{equation}

Trong đó $\Gamma$ là hàm Gamma. Giá trị trung bình và phương sai của Beta Distribution là:
...
\begin{itemize}
    \item \textbf{Hỗ trợ hành động liên tục trên [0, 1]}: Phù hợp cho các bài toán yêu cầu hành động là tỷ lệ, xác suất hoặc giá trị chuẩn hóa.
    \item \textbf{Linh hoạt}: Có thể biểu diễn nhiều dạng phân bố (đơn điểm, đều, bán nguyệt, v.v.) bằng cách điều chỉnh $\alpha$ và $\beta$.
    \item \textbf{Khuyến khích exploration}: Phương sai của Beta Distribution cho phép mô hình sample các hành động khác nhau, giúp khám phá không gian hành động.
    \item \textbf{Tính toán hiệu quả}: So sánh với các phân bố khác, Beta Distribution có công thức đóng cho pdf và cdf.
\end{itemize}

Trong policy network sử dụng Beta Distribution, actor network thường sản sinh ra các tham số $\mu$ (mean) và $\sigma$ (standard deviation), sau đó chuyển đổi thành $\alpha$ và $\beta$:
...
Hành động được sample từ Beta distribution:

\begin{equation}
a \sim \text{Beta}(\alpha, \beta)
\end{equation}

Và thường được clamp về khoảng $[\epsilon, 1-\epsilon]$ để tránh các giá trị cực đoan, trong đó $\epsilon$ là một giá trị nhỏ (ví dụ: $10^{-6}$).
...
Generator sử dụng kiến trúc mạng Residual Network (ResNet) thay vì các mạng tích chập sâu thông thường. Việc lựa chọn ResNet đóng vai trò quan trọng bởi các kết nối tắt trong kiến trúc này giúp dòng gradient lan truyền hiệu quả hơn trong quá trình huấn luyện. Nhờ đó, mô hình có thể xây dựng các mạng rất sâu mà không gặp phải hiện tượng mất mát gradient, đồng thời cải thiện khả năng học đặc trưng tinh vi trong ảnh.
...
Trong đó:
\begin{itemize}
    \item $\lambda_{coord}$: Hệ số điều chỉnh, thường được đặt giá trị cao hơn để nhấn mạnh tầm quan trọng của localization loss.
...
Trong đó:
\begin{itemize}
    \item $\lambda_{noobj}$: Hệ số điều chỉnh quan trọng được sử dụng để kiểm soát trọng số của phần lỗi liên quan đến các bounding box không chứa đối tượng (no-object).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth,height=0.6\textheight,keepaspectratio]{fig/IOU.jpg}
    \captionsetup{justification=centering}
...
\subsection{Hiệu suất của mô hình YOLOv11}
...
Giao thông đô thị thực tế là hệ thống động, lưu lượng xe và tốc độ phương tiện thay đổi liên tục theo từng giờ trong ngày. Nếu mô phỏng chỉ sử dụng lưu lượng cố định (ví dụ: luôn giả định 50 xe/phút suốt ngày), kết quả sẽ không phản ánh được thực tế: tốc độ sẽ không khác biệt giữa giờ cao điểm và giờ bình thường, các mô hình dự báo sẽ không học được mẫu hành vi thực sự. Để tạo ra một mô phỏng gần gũi với thực tế, cần phải thu thập dữ liệu lưu lượng từ thực tế (vào các giờ khác nhau trong ngày) và tái hiện những biến đổi này trong mô phỏng.

\textbf{Tại sao cần chuyển đổi từ thực tế vào mô phỏng:}

Việc mô phỏng lưu lượng và tốc độ theo thời gian có ý nghĩa quan trọng:

\begin{itemize}
    \item \textbf{Nếu không làm}: Mô phỏng sử dụng lưu lượng cố định hoặc tùy ý sẽ cho ra kết quả không chính xác. Tốc độ giả lập không tương ứng với thực tế, các mô hình dự báo huấn luyện trên dữ liệu này sẽ cho dự báo sai và không thể sử dụng để hỗ trợ quyết định thực tế.
    
    \item \textbf{Nếu làm}: Mô phỏng phản ánh đúng hành vi giao thông thực tế, tốc độ và lưu lượng biến thiên giống như ngoài đường. Điều này tạo dữ liệu huấn luyện chất lượng cao để các mô hình học máy và học sâu (như LSTM, CNN) học được các mẫu thực sự và đưa ra dự báo chính xác.
\end{itemize}

\textbf{Nguyên nhân biến thiên lưu lượng và tốc độ:}

Lưu lượng và tốc độ biến thiên trong mô phỏng do các yếu tố sau:

\begin{itemize}
    \item \textbf{Nhu cầu di chuyển theo giờ}: Lưu lượng xe cao vào giờ cao điểm (sáng, chiều) khi nhiều người cần đi làm hay về nhà, và thấp vào giờ bình thường hay đêm.
    
    \item \textbf{Tác động của mật độ xe}: Khi lưu lượng tăng, các xe xếp gần nhau hơn, buộc phải giảm tốc để duy trì khoảng cách an toàn, từ đó tốc độ trung bình tự động giảm.
    
    \item \textbf{Ảnh hưởng của tín hiệu giao thông}: Tín hiệu đèn giao thông tạo sự ngắt quãng luồng giao thông. Khi đèn đỏ, các xe phải dừng hoàn toàn; khi đèn xanh, các xe tăng tốc. Điều này tạo ra sự biến thiên tốc độ có tính chu kỳ.
\end{itemize}

\textbf{Cách mô phỏng biến thiên lưu lượng và tốc độ:}

Các công cụ mô phỏng giao thông như SUMO thường triển khai biến thiên lưu lượng và tốc độ theo thời gian thông qua các cơ chế sau:

\begin{enumerate}
    \item \textbf{Nhập dữ liệu lưu lượng theo thời gian}: Cho phép định nghĩa lưu lượng khác nhau cho các khoảng thời gian khác nhau (ví dụ: 6:00-9:00 có 100 xe/phút, 9:00-17:00 có 30 xe/phút, 17:00-20:00 có 80 xe/phút). Dữ liệu này thường được thu thập từ thực tế.
    
    \item \textbf{Mô hình hành vi lái xe}: SUMO sử dụng mô hình car-following để điều chỉnh tốc độ của từng phương tiện. Khi xe tiếp cận xe phía trước, tốc độ sẽ tự động giảm để duy trì khoảng cách an toàn. Hành vi này tạo ra sự giảm tốc tự động khi lưu lượng tăng.
    
    \item \textbf{Tích hợp tín hiệu đèn}: Mô phỏng kết hợp chương trình tín hiệu giao thông vào hành vi của xe. Khi gặp đèn đỏ, xe dừng; khi gặp đèn xanh, xe tiếp tục hoặc tăng tốc. Chu kỳ tín hiệu cố định tạo ra những sóng giao thông có tính chu kỳ.
\end{enumerate}

Kết quả là tốc độ trung bình sẽ thấp nhất vào giờ cao điểm khi lưu lượng cao, và cao hơn vào giờ bình thường khi lưu lượng thấp, phản ánh đúng hiện tượng trong giao thông thực tế.

\textbf{Ứng dụng trong dự báo:}

Dữ liệu lưu lượng và tốc độ theo thời gian từ mô phỏng cung cấp bộ dữ liệu huấn luyện quan trọng cho các mô hình dự báo (đặc biệt là mô hình LSTM và CNN được trình bày ở các mục trước). Thông qua dữ liệu này, các mô hình sẽ học được:

\begin{itemize}
    \item Mẫu thay đổi tốc độ theo giờ trong ngày (mẫu chu kỳ hàng ngày).
    \item Mối quan hệ giữa lưu lượng cao và tốc độ thấp.
    \item Cách tính toán dự báo dựa trên lịch sử tương tự từ các ngày trước đó.
\end{itemize}

Nhờ vậy, mô phỏng chính xác của sự biến thiên lưu lượng và tốc độ theo thời gian không chỉ tạo ra bộ dữ liệu huấn luyện chất lượng cao mà còn giúp các mô hình dự báo nắm bắt được bản chất động của giao thông đô thị, từ đó đưa ra dự báo đáng tin cậy cho việc quản lý giao thông.
