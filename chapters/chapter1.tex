...
\begin{itemize}
    \item Xây dựng quy trình thu thập và xử lý ảnh snapshot từ camera giao thông, chuyển đổi dữ liệu hình ảnh thô thành các thông số giao thông có cấu trúc như lưu lượng xe, mật độ phương tiện theo thời gian thông qua các mô hình học sâu.
    ...
    \item Thiết kế và đánh giá chiến lược điều chỉnh tín hiệu đèn giao thông dựa số lượng phương tiện hiện tại, nhằm tối ưu hóa các chỉ tiêu vận hành như thời gian chờ trung bình tại nút giao, số lần dừng xe và mức độ ùn tắc.
...
\begin{itemize}
    ...
    \item Xây dựng mô hình dự báo mật độ giao thông: Phát triển mô hình học sâu dựa trên kiến trúc Long Short-Term Memory (LSTM) để xử lý dữ liệu chuỗi thời gian không gian. Mô hình tiếp nhận dữ liệu vận tốc, mật độ từ các cụm giao thông để dự báo mức độ tắc nghẽn (gridlock levels) trong tương lai gần, làm cơ sở cho các quyết định điều tiết chủ động.
    
    \item Xây dựng mô hình điều chỉnh tín hiệu đèn giao thông: Thiết kế các tác tử học tăng cường (Reinforcement Learning Agents), cụ thể là DurationAgent, để điều khiển tín hiệu đèn giao thông. Agent học chiến lược tối ưu hóa thời lượng pha xanh (Green Duration) một cách thích ứng dựa trên trạng thái giao thông hiện tại và dự báo, thay thế cho các chu kỳ đèn cố định truyền thống.
    
    \item Tích hợp mô phỏng giao thông với SUMO: Xây dựng môi trường mô phỏng vi mô sử dụng phần mềm SUMO (Simulation of Urban Mobility), tích hợp bản đồ thực tế từ OpenStreetMap. Thiết lập giao diện điều khiển qua TraCI (Traffic Control Interface) cho phép đồng bộ hóa dữ liệu thời gian thực và thực thi các lệnh điều khiển từ mô hình Python vào môi trường mô phỏng.
    
    \item Tích hợp module điều khiển giao thông: Kết nối các thành phần rời rạc thành một hệ thống khép kín (Closed-loop System). Dữ liệu từ môi trường mô phỏng được đưa vào mô hình dự báo, kết quả dự báo hỗ trợ Agent ra quyết định điều chỉnh tín hiệu, và các quyết định này được phản hồi ngược lại môi trường để tác động trực tiếp lên luồng giao thông, tạo thành cơ chế điều khiển thích nghi liên tục.
...
\begin{itemize}
    \item \textbf{Đánh giá dữ liệu thu thập từ camera: } Kiểm tra xem số lượng ảnh thu thập có đầy đủ và liên tục trong các khung giờ nghiên cứu.  
    \item \textbf{Đánh giá độ chính xác của khối phát hiện và đếm phương tiện từ camera: } Kết quả đếm phương tiện tự động được đối chiếu với số liệu đếm thủ công, nhằm xác định mức độ chính xác và độ tin cậy của mô hình trong các điều kiện giao thông khác nhau.
    
    \item \textbf{Phân tích hiệu năng của mô hình LSTM: } Đánh giá độ chính xác của mô hình dự báo thông qua các chỉ số sai số (MAE, RMSE) và khả năng phân loại mức độ tắc nghẽn (Accuracy, F1-Score). So sánh hiệu quả giữa các kiến trúc mô hình khác nhau để lựa chọn cấu hình tối ưu.
    
    \item \textbf{Phân tích kết quả mô phỏng giao thông bằng SUMO: } Thực hiện các kịch bản mô phỏng so sánh giữa trường hợp không điều khiển (Baseline - đèn cố định) và có điều khiển thông minh (DurationAgent). Thu thập và phân tích các chỉ số vận hành mạng lưới.
    
    \item \textbf{Đánh giá tổng thể hệ thống: } Tổng hợp các kết quả để đánh giá mức độ cải thiện về các chỉ số hiệu năng chính (KPIs) như: giảm độ dài hàng đợi trung bình, tăng thông lượng (throughput) toàn mạng lưới, và cải thiện điểm thưởng (reward) của hệ thống điều khiển.
