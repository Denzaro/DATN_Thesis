...
\begin{itemize}
    ...
    \item DurationAgent thường tăng thời lượng xanh khi lưu lượng cao (giảm queue), giảm khi lưu lượng thấp (tiết kiệm thời gian).
\end{itemize}
...
\begin{figure}[H]
    ...
    \captionsetup{justification=centering}
    \caption[So sánh độ dài hàng đợi: Model vs Baseline]{So sánh độ dài hàng đợi. DurationAgent (màu cam) đạt độ dài hàng đợi thấp hơn đáng kể so với baseline (màu xám), đặc biệt là trong các khoảng thời gian cao điểm}
    \label{fig:results_queue_length_comparison}
\end{figure}

\textbf{Kết Quả Chính:}
\begin{itemize}
    \item \textbf{DurationAgent:} Độ dài hàng đợi trung bình $\approx$ 13.1 vehicles
    \item \textbf{Baseline:} Độ dài hàng đợi trung bình $\approx$ 18.5 vehicles
    \item \textbf{Cải thiện:} $\frac{18.5 - 13.1}{18.5} \times 100\% = \mathbf{-29.2\%}$ (giảm hàng đợi)
    \item Peak performance: DurationAgent giữ queue $< 15$ xe, trong khi baseline lên tới 25+ xe
    \item Stability: Variance của DurationAgent ($\sigma = 4.2$) thấp hơn baseline ($\sigma = 8.5$)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{DurationAgent:} Reward trung bình $\approx 4388.3$ (peak episode 1011)
    \item \textbf{Baseline:} Reward trung bình $\approx 3204.9$
    \item \textbf{Cải thiện:} $\frac{4388.3 - 3204.9}{3204.9} \times 100\% = \mathbf{+36.9\%}$
...
\begin{itemize}
    ...
    \item Thời gian chờ trung bình giảm (27 phút baseline → 14.5 phút model)
    \item Throughput cao hơn → Nhiều xe qua được giao lộ
...
\begin{itemize}
    \item \textbf{DurationAgent:} Thông lượng trung bình $\approx$ 2847 vehicles/hour
    \item \textbf{Baseline:} Thông lượng trung bình $\approx$ 1820 vehicles/hour
    \item \textbf{Cải thiện:} $\frac{2847 - 1820}{1820} \times 100\% = \mathbf{+56.4\%}$
    \item Peak hours: DurationAgent đạt 1400+ vehicles, baseline chỉ 600 vehicles
    \item Ổn định hơn: Ít biến động trong các giai đoạn vắng (tránh tình trạng "rỗi")
\end{itemize}
...
\begin{table}[H]
    ...
    \hline
    \textbf{Chỉ Số} & \textbf{Baseline} & \textbf{DurationAgent} & \textbf{Cải Thiện} \\
    \hline
    Độ dài hàng đợi (vehicles) & 18.5 & 13.1 & $-29.2\%$ \\
    Thời gian chờ (giây) & 510.2 & 272.85 & $-46.5\%$ \\
    Thông lượng (v/h) & 1820 & 2847 & $+56.4\%$ \\
    Reward trung bình & 3204.9 & 4388.3 & $+36.9\%$ \\
    Queue stability ($\sigma$) & 8.5 & 4.2 & $-50.6\%$ \\
    Teleports (xe bị kẹt) & 15 & 0 & Tất cả tránh được \\
    \hline
    \end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{Độ dài hàng đợi:} DurationAgent giảm 29.2\%, chứng tỏ ít xe xếp hàng. Mặc dù queue không phải chỉ số tốt nhất (xem throughput), nhưng khi kết hợp với throughput cao, nó có nghĩa: "ít xe chờ VÀ nhiều xe đi qua" → hiệu quả tối ưu.
    
    \item \textbf{Thời gian chờ:} Giảm 46.5\% (510 giây → 272 giây), người lái xe chờ lâu hơn ở baseline.
    
    \item \textbf{Thông lượng (Throughput):} Tăng 56.4% (1820 → 2847 v/h), tức DurationAgent cho phép **56\% nhiều xe qua giao lộ**, điều này là ưu tiên cao nhất trong quản lý giao thông.
    
    \item \textbf{Teleports (xe bị kẹt):} Baseline có 15 xe phải "teleport" (bị kẹt quá lâu, SUMO di chuyển chúng). DurationAgent có 0, tức **không xe nào bị kẹt** → điều kiện giao thông an toàn, không gridlock.
    
    \item \textbf{Queue Stability:} Variance giảm 50.6%, chứng tỏ hiệu suất ổn định, không có biến động lớn.
\end{itemize}
...
\begin{table}[H]
    \centering
    \caption{So sánh hiệu suất từng giao lộ giữa Baseline (fixed signal 30s) và DurationAgent}
    \label{table:per_tls_comparison}
    \begin{tabular}{|l|c|c|c|c|}
    \hline
    \textbf{Giao Lộ ID} & \textbf{Baseline} & \textbf{Model} & \textbf{Cải Thiện} & \textbf{Gain \%} \\
    \hline
    TLS 1 (4989...) & 19.19 & 21.73 & +2.54 & +13.3\% \\
    TLS 2 (12320...) & 9.14 & 14.91 & +5.76 & +63.1\% \\
    TLS 3 (10118...) & 19.76 & 22.70 & +2.94 & +14.9\% \\
    TLS 4 (366458462) & -0.10 & -0.12 & -0.02 & -20.0\% \\
    TLS 5 (12310...) & 24.37 & 23.69 & -0.68 & -2.8\% \\
    \hline
    \textbf{Mean} & \textbf{14.47} & \textbf{16.58} & \textbf{+2.11} & \textbf{+13.7\%} \\
    \hline
    \end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{TLS 2 (cluster\_12320...)}: **Cải thiện tốt nhất (+63.1\%)**. Giao lộ này ban đầu có reward thấp (9.14), DurationAgent điều chỉnh hiệu quả lên 14.91. Chứng tỏ baseline cố định 30 giây rất không phù hợp với tình huống giao thông tại đây - thể hiện giá trị thích ứng của mô hình.
    
    \item \textbf{TLS 1 (4989...) \& TLS 3 (10118...)}: **Cải thiện ổn định (+13.3\% và +14.9\%)**. Các giao lộ này đã có reward baseline khá tốt (19+), DurationAgent tiếp tục cải thiện hơn nữa nhờ khả năng thích ứng linh hoạt với biến đổi lưu lượng.
    
    \item \textbf{TLS 5 (12310...)}: **Hơi giảm (-2.8\%)**. Giao lộ này có baseline reward cao nhất (24.37), và DurationAgent giảm xuống 23.69. Điều này có thể vì giao lộ này đã được tối ưu hóa tốt với signal cố định 30 giây, hoặc agent vẫn đang tinh chỉnh chiến lược.
    
    \item \textbf{TLS 4 (366458462)}: **Tệ hơn (-20.0\%)**. Cả baseline và model đều cho reward rất thấp (âm). Model thậm chí tệ hơn baseline một chút (-0.12 < -0.10), điều này có thể do giao lộ này có capacity rất thấp (chỉ 2 làn), lưu lượng rất cao, nên dù có điều chỉnh tín hiệu cũng không giải quyết được vấn đề tắc nghẽn cốt yếu - đây là giới hạn của cấu trúc mạng chứ không phải điều khiển.
    
    \item \textbf{**Trung bình tất cả 5 giao lộ: +13.7\% cải thiện**}. Mặc dù TLS 4 và 5 không cải thiện (hoặc cải thiện tối thiểu), nhưng tổng thể hệ thống vẫn tăng hiệu suất đáng kể. Điều này chứng tỏ DurationAgent hiệu quả trên hầu hết các loại tình huống giao thông.
\end{itemize}
...
\subsection{Limitations và Future Work}

\subsubsection{Hạn Chế Hiện Tại}

\begin{itemize}
    \item \textbf{Adaptation to Sudden Incidents:} Agent cần 30-60 giây để adapt sau khi có incident (xe tai nạn, lane closure).
    \begin{itemize}
        \item \textit{Giải pháp:} Thêm incident detection module từ camera hoặc traffic monitoring system.
    \end{itemize}
    \item \textbf{Multi-TLS Coordination:} Hiện tại các agents hoạt động độc lập, chưa có coordination giữa các giao lộ.
    \begin{itemize}
        \item \textit{Giải pháp:} Sử dụng Graph Neural Networks (GNN) để mô hình hóa các tương tác giữa giao lộ.
    \end{itemize}
    \item \textbf{Extreme Congestion:} Khi flow vượt capacity của đường (gridlock), mô hình vẫn có hiệu suất hạn chế.
    \begin{itemize}
        \item \textit{Giải pháp:} Bổ sung module "demand management" (chuyển hướng luồng).
    \end{itemize}
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Multi-Agent Coordination:} Mở rộng từ single TLS sang network-wide coordination sử dụng multi-agent RL.
    \item \textbf{Real-World Validation:} Test trên dữ liệu giao thông thực tế từ camera và sensors.
    \item \textbf{Weather Integration:} Thêm dữ liệu thời tiết (mưa, sương mù) ảnh hưởng đến lưu lượng.
    \item \textbf{Vehicle Classification:} Phân biệt bus, taxi, private car → điều khiển tối ưu hơn.
    \item \textbf{Predictive Control:} Dự báo 10-15 phút trước để planning proactive (không chỉ reactive).
\end{enumerate}

\subsection{Kết Luận}

DurationAgent sử dụng LSTM và Actor-Critic training đã chứng minh hiệu quả trong việc giảm queue (-29.2\%), giảm thời gian chờ (-46.5\%) và tăng thông lượng (+56.4\%) so với baseline fixed signal. Mô hình hội tụ tốt, ổn định, và có thể triển khai thực tế. Các cải tiến trong tương lai sẽ tập trung vào multi-TLS coordination và integration với dữ liệu thực tế.
